...
################################
### FIGURE 10 depth profiles ###
################################
    
png(file=paste(path_figures,"Figure10_Conc_IOPs_profiles_depths_2011_2014.png",sep=""),
    width=1000, height=600, pointsize=22, family="Helvetica") 
    par(mfrow=c(1,4))
...
##################################   
    plot(x=1, y=1, type="n", col=colores[i], lwd=1, las=1, cex.lab=1.0, pch=19, cex=0.2,
         main="", ylab="m", xlab=expression(paste("TChla (mg ", m^-3,")", sep="  ")),
    ...
    ############
    mtext(3, at=0.5, line=6, text=expression(paste(B[C]," (mgC ", m^-3,")", sep="  ")), cex=0.75, outer=F)
...
##################################  
    ...
    legend(x="bottomright",legend=c("TChla",(expression(paste(a[CDOM],"(450)"))),expression(B[C]),"DOC"),
           col=c(linea_phyto, linea_cdom, linea_bact, linea_grey), bty="n", lwd=3) 
...
#################
### FIGURE 11 ### Concentrations seasonal
#################

png(file=paste(path_figures,"Figure11_Conc_IOPs_depths_2011_2014.png",sep=""),
    width=1300, height =370, pointsize=22, family="Helvetica") 
    par(mfrow=c(1,2))
    #layout(matrix(c(1:12),ncol=3, byrow=T), widths = c(1,0.5,0.7))
    #layout(matrix(c(1:6),ncol=3, byrow=T), widths = c(1,0.5,0.7))
    par(mar=c(2,1,1,1))
    par(oma=c(1,7,0,7))
    #layout.show(n=12)
    layout.show(n=2)
...
##################################   
  ...
  plot(x=1, y=1, type="n", col=colores[i], lwd=1, las=1, cex.lab=1.0, pch=19, cex=0.2,
       main="", ylab=expression(paste("TChla (mg ", m^-3,")", sep="  ")), xlab="",
...
####################
      ...
      mtext(2, at=0.7, line=2.5, text=expression(paste("TChla (mg ", m^-3,")", sep="  ")), cex=cexlabs, outer=F)
...
####################       
      ...
      #mtext(4, at=0.5, line=7, text=expression(paste("mgC ", m^-3, sep="  ")), cex=0.75, outer=F)
...
#dcm_depth    # depths
  ...
  legend(x=150,y=1.4,legend=c("TChla",(expression(paste(a[CDOM],"(450)"))),expression(B[C]),"DOC"),
         col=c(linea_phyto, linea_cdom, linea_bact, linea_grey), bty="n", lwd=anchol, cex=0.75)  
          ...
          mtext(4, at=0.7, line=7, text=expression(paste(B[C]," (mgC ", m^-3,")", sep="  ")), cex=cexlabs, outer=F)
...
#################
### FIGURE 12 ### Absorption budget
#################

png(file=paste(path_figures,"Figure12_Absorption_budget_depths_2011_2014.png",sep=""),
    width=1200, height=400, pointsize=22, family="Helvetica") 
    #par(mfrow=c(4,1))
    #layout(matrix(c(1:12),ncol=3, byrow=T), widths = c(1,0.5,0.7))
    ...
    #################
        ...
        plot(x=1, y=1, type="n", col=colores[i], lwd=1, las=1, cex.lab=1.0, pch=19, cex=0.2,
             main="", ylab=expression(paste("%", sep="  ")), xlab="",
...
#################
### FIGURE 13 ### Bio-optical relationships
#################

png(file=paste(path_figures,"Figure13_Bio_optical_relationships_2011_2014_seasons.png",sep=""),width=1200, height=580, pointsize=20, family="Helvetica") 
            par(mfrow=c(1,2))
            par(mar=c(4,4,2,1))
            par(oma=c(0,1,0,0))
            cexletter=1.6
  ...
  #########################
            ...
            colores_estaciones<-rep(c(rep("steelblue3",60),rep(c("yellow3","orchid3","coral3"), each=90),rep("steelblue3",30)),times=4)[-1]
            #cbind(colores_estaciones,colores_estaciones2)
     ...
     legend(x=-3,y=0.06,legend=c("Winter (D-F)","Spring (M-M)","Summer (J-A)","Fall (S-N)"),
            col=unique(colores_estaciones), bty="n", pch=19)  
     
     legend(x="topleft", legend=c(expression(paste("Morel & Gentili (2009)",sep="")),
     ...
     legend(x="bottomright",legend=expression(paste(italic("In situ")," ",BOUSSOLE, sep="")),
      ...
      ### aCDOM(440) vs. DOC      
      plot(x=1, y=1, type="n", lwd=1, las=1, cex.lab=1.0, pch=19, cex=0.2,
                 main="",ylab=expression(paste("DOC (",mu, "molC ", L^-1,")", sep="  ")),
          ...
          polygon(x=data_Gat_depth$x,y=data_Gat_depth$y, col="grey90", border=NA)
            ...
            legend(x="bottomright",legend=c("Winter (D-F)","Spring (M-M)","Summer (J-A)","Fall (S-N)"),
                   col=unique(colores_estaciones), bty="n", pch=19)            
            legend(x="topleft",
