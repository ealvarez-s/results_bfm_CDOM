...
###########################
    ...
    con <- dbConnect(drv=RSQLite::SQLite(),
                     dbname=paste(o_dir,"CALIBRATION/boussole_annual_DE30_OPTM5_1.db", sep=""))
    ## list all tables
    ...
    ######################
    con <- dbConnect(drv=RSQLite::SQLite(),
                     dbname=paste(o_dir,"CALIBRATION/boussole_annual_DE30_OPTM5_2.db", sep=""))
    ## list all tables
    tables <- dbListTables(con)
    ## exclude sqlite_sequence (contains table information)
    tables <- tables[tables != "sqlite_sequence"]
    ...
    ######################
    con <- dbConnect(drv=RSQLite::SQLite(),
                     dbname=paste(o_dir,"CALIBRATION/boussole_annual_DE30_OPTM5_3.db", sep=""))
    ## list all tables
    tables <- dbListTables(con)
    ## exclude sqlite_sequence (contains table information)
    tables <- tables[tables != "sqlite_sequence"]
    ...
    ######################
    con <- dbConnect(drv=RSQLite::SQLite(),
                     dbname=paste(o_dir,"CALIBRATION/boussole_annual_DE30_OPTM5_4.db", sep=""))
    ## list all tables
    tables <- dbListTables(con)
    ## exclude sqlite_sequence (contains table information)
    tables <- tables[tables != "sqlite_sequence"]
    ...
    ######################
    con <- dbConnect(drv=RSQLite::SQLite(),
                     dbname=paste(o_dir,"CALIBRATION/boussole_annual_DE30_OPTM5_5.db", sep=""))
    ## list all tables
    tables <- dbListTables(con)
    ## exclude sqlite_sequence (contains table information)
    tables <- tables[tables != "sqlite_sequence"]
    ...
    save(maxiter,inicio,fin,param1,lnls,valores,extra_outputs,total_runs, file=paste0(o_dir,"CALIBRATION_R1/DE30_OPTM5.RData"))
...
############## 
    ...
    #png(file=paste(path_figures,"FigureA2_params_along_optim_DE30_OPTM5.png",sep=""),width = 1200, height = 800, pointsize=20) 
    
    pdf(file=paste(path_figures,"FigureA2_params_along_optim_DE30_OPTM5.pdf",sep=""),
    ...
    #for (k in c(1:nparams)){
      ...
      if (k>=14 & k<=17) {plot(x_gen,param1,las=1,#ylim=c(minimum[k],maximum[k]),
                          ...
                          mgp=c(3.75,1,0), main="", yaxt="n", type="n", xaxs="i", yaxs="i")
                   ...
                   xaxt="n",xlab="generation", ylab=parameter_units[k], col="grey95",pch=19, cex=0.05,
                   mgp=c(3.75,1,0), main="", type="n", xaxs="i", yaxs="i")}
    ...
    ###################
    mtext(1,line=2.0,at=seq(0.130,0.930,length=5),text=c("generation"),outer=T, cex=cexaxis)
    dev.off()
...
data.frame(NOMBRE,MINIMA,MAXIMA,FINALL,PlusMinus)
    ...
    observations_name<-c(expression("pico Chl-a"),expression("nano Chl-a"),expression("micro Chl-a"),
    ...
    observations_units<-c(rep(expression(paste("(","mg"," ", m^-3,")", sep="")),3),
...
#png(file=paste(path_figures,"FigureA1_metrics_along_optim_DE30_OPTM5.png",sep=""), width = 1200, height = 600, pointsize=20) 
    
pdf(file=paste(path_figures,"FigureA1_metrics_along_optim_DE30_OPTM5.pdf",sep=""),
        width = 1200/90, height = 600/90, pointsize=15, family="Helvetica")     
    ...
    for (k in c(1:7)){
      ...
      mtext(side=3, at=10, line=0.5, text=letritas[k], cex=cexlabel-0.2, font=2)
      ###################
    }
    ##################### 
