...
#png(file=paste(path_figures,"Figure7_timeseries_surface_satellite_fod.png",sep=""),width=1000, height =700, pointsize=24, family="Helvetica")
pdf(file=paste(path_figures,"Figure7_timeseries_surface_satellite_fod.pdf",sep=""),width=1000/100, height =700/100, pointsize=17, family="Helvetica")
    ...
    # Get depth index of first optical depth 
    #### zeu
    par<-filerc$lightspectral_PAR_tot
    zeu<-apply(par[196:1,], MARGIN=2, FUN=find_eudepth, prof=depth_center[196:1], dif=0.01)
    #points(x=tiempo, zeu, type="l", col="white", las=1, ylim=c(200,0))
    #points(x=tiempo, zeu/4.6, type="l", col="green", las=1, ylim=c(200,0))
    index_fop<-rep(NA,lenght=length(zeu))
    for (j in c(1:length(zeu))){
    if (is.na(zeu/4.6)[j]) {index_fop[j]<-171}
      else{diferencia<-abs(depth_center-(zeu/4.6)[j])
      index_fop[j]<-which(diferencia==min(diferencia,na.rm=T))}
    }
    #range(index_fop,na.rm=T)
    #quantile(index_fop, probs=c(0.05,0.95), na.rm=T)
    ...
    plot(x=1, y=1, type="n", col=colores[i], lwd=1, las=1, cex.lab=1.0, pch=19, cex=0.2,
        ...
        mtext(2, at=-0.20, line=2.8, text=expression(paste("TChl-a", sep="")), las=1, cex=cexunits)
        mtext(2, at=-0.50, line=2.8, text=expression(paste("(mg ", m^-3,")", sep="")), las=1, cex=cexunits)
        #legend(x="topleft", legend=expression(paste("TChl-a")), cex=1.2,bty="n")
        ...
        for (q in c(1:ncol(R0))) aph[q]<-mean(R0[(index_fop[q]):196,q],na.rm=T)
        points(x=tiempo, y=log10(aph), type="p", col=colores[i], lwd=1, las=1, cex.lab=1.0, pch=19, cex=0.4)
        #range(aph-aph2,na.rm=T)
...
#          legend(x=1300, y=0.7,legend=c(expression("Chla-fluor")),fill=c("green"), bty="o", border=NA)                
    ...
    plot(x=1, y=1, type="n", col=colores[i], lwd=1, las=1, cex.lab=1.0, pch=19, cex=0.2,
        ...
        #abline(h=0.01,lty=3, col="grey")
        axis(1, at=mid_meses, labels=F, las=1, cex.axis=0.8)
        mtext(2, at=0.056, line=2.8, text=expression(paste(a[PH],"(450)")), las=1, cex=cexunits)
        mtext(2, at=0.042, line=2.8, text=expression(paste("(",m^-1,")")), las=1, cex=cexunits)
        ...
        #legend(x="topleft", legend=expression(paste(a[PH],"(450) ",m^-1)), cex=1.2,bty="n")
...
#########################      
      ...
      # legend(x=1225, y=0.0950,legend=c(expression("Satellite"), expression(paste(italic("In situ")," BOUSSOLE")),
      #                                  expression(italic("Optimized")), expression(italic("Optimized FOD"))),
      #            fill=c(col_sat, col_bou, colores[elegidos], "blue3"), bty="o", border=NA)                
...
### aDG(450)(9m)
        plot(x=1, y=1, type="n", col=colores[i], lwd=1, las=1, cex.lab=1.0, pch=19, cex=0.2,
             ...
             xaxs="i", ylim=c(0,0.1),yaxs="i", xaxt="n")
        axis(1, at=mid_meses, labels=F, las=1, cex.axis=0.8)
        mtext(2, at=0.056, line=2.8, text=expression(paste(a[DG],"(450)")), las=1, cex=cexunits)
        mtext(2, at=0.042, line=2.8, text=expression(paste("(",m^-1,")")), las=1, cex=cexunits)
        ...
        #legend(x="topleft", legend=expression(paste(a[DG],"(450) ",m^-1)), cex=1.2,bty="n")
        ...
        for (q in c(1:ncol(R0))) adg[q]<-mean(R0[(index_fop[q]):196,q],na.rm=T)
        points(x=tiempo, y=adg, type="p", col=colores[i], lwd=1, las=1, cex.lab=1.0, pch=19, cex=0.4)
        #range(adg-adg2,na.rm=T)
